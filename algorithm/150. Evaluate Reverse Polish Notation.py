        ...
        for token in tokens:
            if token in "+-*/":
                ...
                if stack:
                    ...
                    if token == "-": stack.append( lhs - rhs )
                    if token == "*": stack.append( lhs * rhs )
                    if token == "/": stack.append( int(float(lhs) / rhs) )

            else:
                stack.append(token)

        if stack: return int(stack.pop())
